import React, { useState, useRef, useEffect, useCallback, Ref, useLayoutEffect } from 'react';
...
const QuillEditor = () => {
    ...
    const [category, setCategory] = useState<string>("Financial Literacy");
    ...
    const [authorName, setAuthorName] = useState<string>("Layla Lombardi")
    ...
    const handleSubmit = async (event: Event | any) => {
        ...
        let authorSource: Promise<string> = UploadImage(mainImg as File, mainImgName, '/authors/images/');

        let mainImgSource: Promise<string> = UploadImage(mainImg as File, mainImgName, '/blog-posts/images/');

        await Promise.all([mainImgSource, authorSource])

        await CreateAuthor(authorName, await authorSource)
        const authorRef = await db.collection('authors').where("name", "==", authorName).get()
        const author_id = authorRef.docs[0].id

        CreateBlogPost(quillInfo, authorName as string, author_id, await authorSource, category as string, description as string, title as string, imageCaption as string, await mainImgSource)
            ...
            <div className="quill-editor-container">
                <form className="quill-editor-container__form" action="">
                    ...
                    >
                        ...
                        <option value="Micheal Angelo">Micheal Angelo</option>
                    </select>
